...
create table `PROGRAM_CONCLUSION` (`GRADUATION_TITLE` text, `GRADUATION_LEVEL` text, `OID_DOMAIN_META_OBJECT` bigint unsigned, `OID_ROOT` bigint unsigned, `EVENT_TYPES` text, `OID` bigint unsigned, `AVERAGE_EDITABLE` tinyint(1), `ALUMNI_PROVIDER` tinyint(1), `SKIP_VALIDATION` tinyint(1), `TARGET_STATE` text, `NAME` text, primary key (OID), index (OID_ROOT), index (OID)) ENGINE=InnoDB, character set utf8;
