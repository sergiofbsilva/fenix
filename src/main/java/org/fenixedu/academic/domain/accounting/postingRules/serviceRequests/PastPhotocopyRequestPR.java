...
import org.fenixedu.academic.domain.accounting.events.serviceRequests.PastPhotocopyRequestEvent;
...
public class PastPhotocopyRequestPR extends PastPhotocopyRequestPR_Base {

    protected PastPhotocopyRequestPR() {
        super();
    }

    public PastPhotocopyRequestPR(final DateTime startDate, final DateTime endDate,
            final ServiceAgreementTemplate serviceAgreementTemplate) {
        super.init(EntryType.PHOTOCOPY_REQUEST_FEE, EventType.PAST_PHOTOCOPY_REQUEST, startDate, endDate,
                serviceAgreementTemplate);
    }

    @Override
    protected Money doCalculationForAmountToPay(final Event event, final DateTime when, final boolean applyDiscount) {
        return ((PastPhotocopyRequestEvent) event).getPastAmount();
    }
